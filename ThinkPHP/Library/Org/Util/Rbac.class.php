 ...
 */
// 配置文件增加设置
// USERs_AUTH_ON 是否需要认证
// USERs_AUTH_TYPE 认证类型
// USERs_AUTH_KEY 认证识别号
...
class Rbac {
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node.id,node.name from ".
                    ...
                    $table['node']." as node ".
                    "where users.user_id='{$authId}' and users.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1";
        $apps =   $db->query($sql);
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where users.user_id='{$authId}' and users.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1";
            ...
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where users.user_id='{$authId}' and users.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=1";
                    $rs =   $db->query($sql);
                    foreach ($rs as $a){
	...
	static public function getModuleAccessList($authId,$module) {
        ...
        $sql    =   "select access.node_id from ".
                    ...
                    $table['user']." as users,".
                    $table['access']." as access ".
                    "where users.user_id='{$authId}' and users.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  access.module='{$module}' and access.status=1";
        $rs =   $db->query($sql);
        $access	=	array();
        foreach ($rs as $node){
            $access[]	=	$node['node_id'];
